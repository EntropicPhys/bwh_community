...
% Output folder: 'bps1a'
p = bpcontini('s1/T1', 'bpt6', 19, 'bps1a',1e-3);  % Start from bif point with ds = 3e-3
% Plotting setup
p.plot.bpcmp = 5;      % Plot 5th component during continuation
p.nc.dsmax = 1e-2;     % Max step size
p.nc.dsmin = 1e-4;     % Min step size
p.nc.del   = 3e-1;     % finite difference discretization of extra terms
...
p.nc.tol = 1e-7;        % Newton tolerance
p.file.smod = 10;       % Save every 10th solution point
%% === check of dudG^Tpsi matrix ====
p.fuha.spjac=@bpjac;[Gu,Gn]=bpjaccheck(p);
Gd=abs(Gu-Gn); em=max(max(Gd)); 
mclf(20); spy(Gd>1e-3);
%%
% Clean up old files if present and start continuation
huclean(p);             % Remove any previous output files in the folder
p = cont(p, 400);       % Continue for 600 steps
...
ylabel('Wavenumber');
title('Upper Busse Balloon chi=1');
set(gca, 'FontSize', 14);
