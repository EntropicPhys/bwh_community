...
for i=1:N; baa(i)=chi(i)*sum(M*B(:,i))/vol; end; chicent=sum(baa)/sum(ba);
% biomass maxima
chii=linspace(0,1,200); interbiochi=interp1(chi,ba,chii,'spline');
[biomax,idx]=max(interbiochi); chimax=chii(idx); 
try frcut=p.frcut; catch frcut=0.001; end % functional diversity 
[br,ir]=find(ba>frcut); brmin=chi(min(ir)); brmax=chi(max(ir)); 
out=[l1v; l2v; chicent; chimax; bmax0; par]; 