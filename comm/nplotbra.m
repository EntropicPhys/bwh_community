function nplotbra(nnn,varargin) % same as plotbra, with add normalization (1st argument) 
...
bplab=[]; fplab=[]; hplab=[]; 
noa=nargin-1; % number of input-arguments
...
%pp=p.branch(length(bradat(p))+cmp,:); % set y-Data
p.branch(6+cmp,:)=p.branch(6+cmp,:)/nnn; 
pp=p.branch(6+cmp,:); % set y-Data
...
%      plot(p.branch(xcmp,i),p.branch(length(bradat(p))+cmp,i),'o','MarkerSize',ms*lwun,'color',cl); 
 plot(p.branch(xcmp,i),p.branch(6+cmp,i),'o','MarkerSize',ms*lwun,'color',cl); 
    end
    if(ms>0 && ismember(p.branch(2,i),3)) % plot markers for hopf points
      plot(p.branch(xcmp,i),p.branch(6+cmp,i),'d','MarkerSize',ms*lwun,'color',cl); 
    end
    if(fms>0 && ismember(p.branch(2,i),2)) % plot markers for fold points
      plot(p.branch(xcmp,i),p.branch(6+cmp,i),'x','MarkerSize',fms*lwun,'color',cl); 
    end
end
