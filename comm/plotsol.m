...
% mod of library-plotsol cause we need to call oosetfemops for p.mat.M 
...
switch size(po,1)
  ...
  case 3 %%%%%%%%%%%%%%%%%%%%%% 3D %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    ...
    switch pstyle; 
       case 2; is=0; isn=0; % iso-plot, complicated 
       noa=nargin;
...
% if levc does not exist set default and if exist and is given via strings
...
% if two colors are given in levc, do color movement
       ...
       % convert colors from string to rgb code (used for creating a title)
        if ischar(p.plot.levc); p.plot.levc=pde2pathcolors(p.plot.levc); end
        if iscell(p.plot.levc); l=[]; 
          for i=1:length(p.plot.levc); l=[l;  pde2pathcolors(p.plot.levc{i})]; end; p.plot.levc=l; 
        end
       %creating a title
        l=length(p.plot.lev); 
        if l==1; t=['level=\color[rgb]{',num2str(p.plot.levc(1,:)),'}',num2str(p.plot.lev,3)]; 
        else t=['levels=\color[rgb]{',num2str(p.plot.levc(1,:)),'}',num2str(p.plot.lev(1),3)]; 
        for i=2:l
          t=[t,['\color{black}, \color[rgb]{',num2str(p.plot.levc(i,:)),'}',num2str(p.plot.lev(i),3)]];         
        end
        end
  case 1;  slplot(p,p.plot.ng,u,p.plot.fs);  
    try colormap(p.plot.cm); catch, colormap cool;  end;  
    axis image;  colorbar('southoutside');  
  case 3; faceplot(p,u); try colormap(p.plot.cm); catch, colormap cool; end;      
    if shsw~=0; shading interp; end 
    colorbar; %('southoutside'); 
  case 4;  try cut=p.plot.cut; catch; cut=[0 -1e6 -1e6]; end     
    p.pdeo.grid.cutplot(u,cut); 
    try colormap(p.plot.cm); catch, colormap cool; end; 
    colorbar; %('southoutside'); 
end % 3D plot-styles 
end % dimension dependence 
drawnow 
