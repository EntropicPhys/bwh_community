...
% cswibra: branch switching via cubic BE 
...
% ali: 'active' list: 
% besw:    if=0, then don't compute quad.bif.eqns (just kernel) 

aux=[]; 
if ischar(dir); p=loadp(dir,fname);
else; p=dir; if nargin>1; aux=fname; end % input is struct, 
end
...
if m==1; fprintf('******  1D kernel, calling swibra ****** \n'); 
    if isfield(aux,'ndir'); q=swibra(p,aux.ndir); else q=swibra(p); end; return
end
try; fprintf(['lam=' num2str(lam), '; 8 smallest eigenvalues: ' num2str(p.sol.muv(1:8),3) '\n']); 
if m>8; fprintf(['lam=' num2str(lam), '; 9 next eigenvalues: ' num2str(p.sol.muv(9:17),3) '\n']); end; 
catch; fprintf(['lam=' num2str(lam), '; smallest eigenvalues: ' num2str(p.sol.muv(1:length(p.sol.muv)),3) '\n']); 
end
...
par=p.u(p.nu+1:end); % for plotting 
%p.sol.muv(1:m)' 
ali=1:m; try; if ~isempty(aux.ali); ali=aux.ali; end; catch; end; m=length(ali); % select active list 
...
if verb>0
  ...
  for i=1:m
      ...
      plotsolu(p,[phiv(:,i);par],10+i,1,pstyle); title(['phi-' mat2str(i,2)]); 
      %axis(p.plot.axis); xlabel(''); ylabel(''); zlabel(''); xticks([]); yticks([]);       
  end 
end 
drawnow; 
mt=zeros(p.nu+1+p.nc.nq,m); % store kernel vectors for gentau in p.mat.tau 
...
alv=[]; nsol=0; 
try; opt=optimoptions('fsolve','Display','off','TolFun',soltol^2); % matlab 
...
p.mat.ctau=[]; p.mat.pred=[]; % storage for tangents 
...
for i=1:size(alsv,1)
   ...
   if iso && fvn<10*soltol && (norm(al)>almin); % was norm(fval)<10*soltol
     ...
     if new
       ...
       alv=[alv;[bet, al, fvn,ndjac]]; 
       p.mat.ctau=[p.mat.ctau, tau]; % store tangent
       p.mat.pred=[p.mat.pred, pred]; % store pred 
     end 
   end
end
end
...
if verb>0
  ...
  for i=1:min(nsol,npmin) 
    ...
    d=0.01/(1+m2-m1); 
    plotsolu(p,up,20+i,1,pstyle); title(['sign(\beta)=' mat2str(sign(alv(i,1)),2)]); xlabel(''); ylabel(''); 
  ...
  if nsol>npmin; ptau=0; ptau=asknu('Also plot the remaining tau?', ptau); 
    if ptau
      for i=npmin+1:nsol 
         ...
         plotsolu(p,up,p.plot.ifig,1,pstyle); 
